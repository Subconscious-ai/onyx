...
Hosted follow-up: the first final preview preparation failed closed on an invalid provider source index. The extraction tool previously advertised indices through 79 even for a one-message conversation. The tool schema now enumerates only actual transcript indices; a distinguishing regression rejects an absent index and confirms per-request schema isolation. Ten structured-brief regressions pass. Hosted automatic preparation and paired transfer must pass before the QA update.
