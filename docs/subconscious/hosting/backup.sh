...
resume() {
  /opt/burn/app/docs/subconscious/hosting/resume-writers.sh || true
  rm -rf "$backup_dir"
}
trap resume EXIT
...
docker exec burn2-relational_db-1 psql -U postgres -d postgres -Atc "SELECT 'users',count(*) FROM \"user\" UNION ALL SELECT 'chats',count(*) FROM chat_session UNION ALL SELECT 'messages',count(*) FROM chat_message;" > "$backup_dir/counts.txt"
timeout 60s docker run --rm -v burn2_minio_data:/data:ro -v "$backup_dir:/backup" alpine:3.21 tar -cf /backup/objects.tar -C /data .
...
printf "Writers resumed after %s seconds\n" "$SECONDS"
