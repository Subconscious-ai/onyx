...
For analytics, use the narrower reminder: “Complete the latest substantive customer request using actual tool results. Numeric charts use source values and native image files, never causal arrows. Preserve simulated-evidence and missing-data limits. Never invent tool calls, unavailable artifacts, or file links.” The September 12 replay found a broad diagram reminder displacing a numeric bar-chart request. The repaired prompt returned actual native PNG files with source values and simulation warnings.

Persona prompts live in native Postgres. A Git push or API image replacement does not update stored prompts. After a prompt change, update the existing native persona and compare exact prompt text. Preserve tool assignments, model, ownership, document sets and shares. Beca uses [executive-turn-reminder.md](executive-turn-reminder.md); analytics uses [experiment-analytics-prompt.md](experiment-analytics-prompt.md) plus the analytics reminder above.
...
The model handoff destination for the Beca prototype branch is `https://causl-scenarios-558-subconcious.vercel.app/dashboard/burn-import`. Preserve exact origin checks. Full interview/model reliability evidence remains in [the prototype plan](prototype-plan.md) and paired draft PRs.
